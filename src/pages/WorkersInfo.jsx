...
const ROLES = ['Content Specialist', 'Editor', 'Video Grapher', 'Meta Ads', 'Software Developer', 'Manager']
...
export default function WorkersInfo() {
  ...
  const filteredWorkers = useMemo(() => {
    return (workers || []).filter(w => {
      return !search || w.name?.toLowerCase().includes(search.toLowerCase()) || w.role?.toLowerCase().includes(search.toLowerCase())
    }).sort((a, b) => {
      const aTL = a.level === 'Team Lead' || a.isTeamLead
      const bTL = b.level === 'Team Lead' || b.isTeamLead
      if (aTL && !bTL) return -1
      if (!aTL && bTL) return 1
  ...
  const handleSave = () => {
    ...
    const payload = {
      ...
      isTeamLead: form.level === 'Team Lead'
    }
  ...
  const handleDelete = (id) => {
    if (window.confirm('Are you sure to remove this employee profile?')) deleteWorker(id)
  }
  ...
  return (
    <div className="w-full flex-1 min-h-screen bg-background flex flex-col">
          ...
          <div className="flex flex-col sm:flex-row items-center justify-between gap-4 sm:gap-6">
            <div className="text-center sm:text-left">
              <h1 className="text-xl sm:text-2xl md:text-3xl font-medium text-white tracking-tighter truncate">
                People <span className="text-primary/50">Registry</span>
              </h1>
                  ...
                  <tr key={w.id} className="hover:bg-sidebar transition-colors group">
                    ...
                    <td className="px-8 py-4 border-r border-border">
                      <div className="flex items-center gap-2.5">
                        ...
                        <span className="text-[13px] font-medium text-secondary whitespace-nowrap">{w.role}</span>
                      </div>
                    </td>
                ...
                filteredWorkers.map((w) => (
                  <div key={w.id} className="group bg-sidebar/30 hover:bg-sidebar/50 border border-white/5 rounded-xl p-5 sm:p-6 space-y-5 transition-all duration-500 hover:shadow-2xl hover:shadow-primary/5 active:scale-[0.99] relative overflow-hidden ring-1 ring-white/5">
                    ...
                    <div className="bg-black/20 backdrop-blur-3xl border border-white/5 rounded-xl p-4 sm:p-5 group-hover:border-primary/20 transition-all duration-500 flex items-center justify-between">
                      <div className="space-y-1">
                        <p className="text-[9px] text-muted font-medium opacity-50 tracking-widest leading-none">Joined On</p>
                        <div className="flex items-center gap-2 text-white font-medium text-sm sm:text-base tracking-tighter tabular-nums drop-shadow-2xl">
      ...
      <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title={selectedWorker ? "Update Person Profile" : "Register New Employee"} size="md">
          ...
          <div className="grid grid-cols-1 gap-6">
            ...
            <div className="grid grid-cols-2 gap-5">
              <FormField label="Assign Role">
                <select
                  className="bg-sidebar border border-white/10 h-14 w-full px-5 rounded-xl text-xs font-medium text-white outline-none focus:border-primary/40 appearance-none transition-all shadow-inner"
                  value={form.role || ''}
      ...
      {/* Footer Area */}
      <div className="p-8 sm:p-12 flex justify-center bg-panel border-t border-border">
        <button
        ...
        >
          ...
          <span className="text-[11px] font-medium text-white opacity-80 group-hover:opacity-100 transition-opacity whitespace-nowrap">Return to Workspace</span>
        </button>
      </div>

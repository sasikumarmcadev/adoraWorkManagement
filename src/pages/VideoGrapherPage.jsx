...
function CalendarView({ tasks, deleteTask }) {
  ...
  return (
    <div id="calendar-view" className="w-full flex-1 p-0 overflow-hidden flex flex-col bg-panel">
      ...
      <Modal isOpen={!!selectedTasks} onClose={() => setSelectedTasks(null)} title={`Shoots for ${selectedTasks?.date}`} size="md">
    ...
    <div className="flex-1 p-4 sm:p-8 overflow-y-auto">
      <div className="max-w-6xl mx-auto">
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-6 sm:mb-8 border-b border-border pb-4 gap-3">
          <div>
            <h1 className="text-2xl sm:text-3xl font-bold text-white tracking-tight">Video Grapher Management</h1>
          ...
          <div className="bg-primary/10 border border-primary/20 px-3 sm:px-4 py-1.5 sm:py-2 rounded-md">
            <span className="text-primary font-bold text-sm sm:text-base">{videoGraphers.length}</span>
            <span className="text-muted text-[10px] sm:text-xs ml-2 tracking-wider">Active Graphers</span>
          </div>
        </div>

        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
          {videoGraphers.map(name => {
            const grapherTasks = tasks.filter(t => t.workerName === name)
            const doneCount = grapherTasks.filter(t => t.status === 'Done').length
              ...
              >
                <div className="flex items-center gap-3 sm:gap-4 mb-3 sm:mb-4">
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-2">
                      <p className="text-[9px] sm:text-[10px] text-muted tracking-widest font-bold">Capture Specialist</p>
                ...
                <div className="grid grid-cols-2 gap-3 sm:gap-4 mt-auto">
                  <div className="bg-[#111] p-2 sm:p-3 rounded border border-white/5">
                    <p className="text-[8px] sm:text-[10px] text-muted font-bold mb-1">Total Shoots</p>
                    <p className="text-lg sm:text-xl font-bold text-white">{grapherTasks.length}</p>
                  </div>
                  <div className="bg-[#111] p-2 sm:p-3 rounded border border-white/5">
                    <p className="text-[8px] sm:text-[10px] text-muted font-bold mb-1">Pending</p>
                    <p className="text-lg sm:text-xl font-bold text-orange-400">{grapherTasks.length - doneCount}</p>
                  </div>
                </div>
...
export default function VideoGrapherPage() {
  ...
  const grapherTasks = tasks.filter(t => {
    ...
    <div className={`w-full flex-1 flex flex-col p-0 m-0 overflow-hidden ${selectedGrapher ? 'h-screen' : ''}`}>
      ...
      <div className="flex-1 overflow-y-auto">
        <div className="w-full flex flex-col px-0">
          ...
          <div className="min-h-[400px] sm:min-h-[500px] h-[500px] sm:h-[600px] border-b border-border flex flex-col">
            <CalendarView
              tasks={grapherTasks.filter(t => t.clientApproval === 'Approval' || t.clientApproval === 'Completed' || t.clientApproval === 'No Changes' || t.status === 'Done' || t.contentCheck || !t.clientApproval)}
              deleteTask={deleteTask}
            />
          </div>

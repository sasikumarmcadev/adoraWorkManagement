  ...
  return (
    <div className="flex flex-col gap-6 p-6 lg:p-8 bg-[#0a0a0a] border-r border-border h-full">
      <div className="flex items-center gap-3">
        ...
        <h3 className="font-bold text-white text-lg tracking-tight">Ops Intel</h3>
      </div>
      <div className="space-y-4">
        <div className="bg-primary/10 border border-primary/20 rounded-2xl p-6 shadow-2xl">
          <h4 className="text-[10px] text-primary font-bold mb-2">Portfolio Volume</h4>
        ...
        <div className="bg-surface-800/20 p-6 rounded-2xl border border-border">
          <p className="text-[10px] text-muted font-bold mb-4">Roster pulse</p>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        {[
          ...
          <div key={label} className="p-4 rounded-xl bg-[#0a0a0a] border border-border">
            <p className="text-[10px] font-bold text-muted capitalize">{label}</p>
            <p className="text-sm font-bold text-white mt-1">{value}</p>
          </div>
        ))}
      </div>
      <div className="pt-4 border-t border-border">
        <p className="text-xs font-bold text-muted mb-4">Production queue ({clientTasks.length})</p>
        <div className="space-y-2">
          {clientTasks.map(t => (
            <div key={t.id} className="flex justify-between items-center p-3 rounded-xl bg-[#0a0a0a] border border-border hover:bg-sidebar transition-colors">
              ...
              <span className={`${getStatusClass(t.status)} text-[9px] font-bold capitalize px-2 py-1 rounded`}>{t.status}</span>
            </div>
          ))}
        </div>
      </div>
  ...
  return (
    <div className="w-full min-h-screen text-primary bg-background pb-32">
      ...
      <div className="lg:grid lg:grid-cols-4">
        ...
        <div className="w-full lg:col-span-3">
          <div className="flex flex-col sm:flex-row items-center justify-between gap-4 p-4 sm:p-6 lg:p-8 bg-[#0a0a0a] border-b border-border shadow-xl">
            <div>
              <h2 className="text-xl sm:text-2xl font-bold text-white tracking-tight">Core Partnership Database</h2>
            ...
            {canEdit && <button onClick={openAdd} className="group flex items-center gap-2 pl-1.5 pr-6 py-1.5 bg-black/40 hover:bg-black/60 backdrop-blur-3xl border border-white/10 rounded-full transition-all shadow-2xl"><div className="w-9 h-9 rounded-full bg-primary flex items-center justify-center text-black group-hover:scale-110 transition-transform"><Plus size={20} /></div><span className="text-xs font-bold text-white">Initialize Record</span></button>}
          </div>
            ...
            {filteredData.map(c => (
              <div key={c.id} className="group flex flex-col p-6 bg-[#0a0a0a] border border-border hover:border-primary/50 transition-all rounded-3xl cursor-pointer relative overflow-hidden shadow-xl" onClick={() => setDetailItem(c)}>
                <div className="flex items-center gap-4 mb-8">
                  <div className="w-14 h-14 rounded-2xl bg-surface-800 border border-border flex items-center justify-center text-primary font-bold text-2xl group-hover:scale-110 transition-transform">{c.name.charAt(0)}</div>
                  <div><h3 className="text-lg font-bold text-white group-hover:text-primary transition-colors tracking-tight">{c.name}</h3><p className="text-[10px] text-muted font-bold flex items-center gap-2"><span className="w-1 h-1 rounded-full bg-muted/40" />{c.city || 'Global Portfolio'}</p></div>
                </div>
                <div className="grid grid-cols-2 gap-4">
                  <div className="bg-[#111] p-4 rounded-2xl border border-white/5 transition-colors"><p className="text-[9px] text-muted font-bold mb-1 opacity-50">Retainer</p><p className="text-lg font-bold text-emerald-400">{formatCurrency(c.budget || 0)}</p></div>
                  <div className="bg-[#111] p-4 rounded-2xl border border-white/5 transition-colors"><p className="text-[9px] text-muted font-bold mb-1 opacity-50">Units</p><p className="text-lg font-bold text-white">{tasks.filter(t => t.clientId === c.id).length} Active</p></div>
                </div>
                <div className="mt-8 pt-4 border-t border-white/5 flex items-center justify-between">
                  <span className={`${getStatusClass(c.activeStatus)} text-[9px] font-bold capitalize px-2 py-1 rounded shadow-sm ring-1 ring-inset ring-white/10`}>{c.activeStatus}</span>
      ...
      <Modal isOpen={showModal} onClose={() => setShowModal(false)} title={editItem ? 'Update Client Profile' : 'Configure New Client'} size="lg">
        ...
        <div className="flex flex-col sm:flex-row gap-4 justify-end mt-12 border-t border-white/5 pt-8"><button className="btn-secondary px-8 h-[48px] font-bold text-xs" onClick={() => setShowModal(false)}>Discard</button><button className="btn-primary px-10 h-[48px] font-bold text-xs" onClick={handleSave}>{editItem ? 'Commit Update' : 'Initialize Partnership'}</button></div>
      </Modal>

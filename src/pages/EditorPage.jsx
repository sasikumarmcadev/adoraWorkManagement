...
function CoverHeader({ title, onExpandSidebar, isSidebarCollapsed }) {
  const [coverData, setCoverData] = useState(() => {
    const saved = localStorage.getItem('editorCover')
    return saved ? JSON.parse(saved) : {
      img: 'https://images.unsplash.com/photo-1542281286-9e0a16bb7366?auto=format&fit=crop&q=80&w=2000',
      positionY: 50
    }
  })
  ...
  const saveCover = (data) => {
    setCoverData(data)
    localStorage.setItem('editorCover', JSON.stringify(data))
...
function AddTaskModal({ isOpen, onClose, onSave, clients, editTask }) {
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title="Add Editor Task" size="md">
    ...
    <div className="flex-1 p-4 sm:p-8 overflow-y-auto">
      <div className="max-w-6xl mx-auto">
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-6 sm:mb-8 border-b border-border pb-4 gap-3">
          <div>
            <h1 className="page-heading-lg">Editor Management</h1>
...
export default function EditorPage() {
  ...
  return (
    <div className={`w-full flex-1 flex flex-col p-0 m-0 overflow-hidden ${selectedEditor ? 'h-screen' : ''}`}>
      <div className="flex-none sticky top-0 z-40 bg-background shadow-2xl">
        <CoverHeader
          title={selectedEditor}
        ...
        <div className="w-full flex flex-col px-0">
          ...
          {(!isMobile || activeTab === 'worklist') && (
            <div className="w-full border-b border-border">
              <DayWiseWorkList tasks={editorTasks} />
            </div>
          )}
